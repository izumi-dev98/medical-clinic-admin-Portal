...
export default async function handler(request, response) {
  ...
  const username = String(Array.isArray(fields.username) ? fields.username[0] : fields.username || '').trim().toLowerCase()
  const password = String(Array.isArray(fields.password) ? fields.password[0] : fields.password || '')
  const { data: user, error } = await client.from('admin_users').select('id, full_name, username, password_hash, active').eq('username', username).maybeSingle()
  if (error) return response.status(500).json({ error: `Could not read admin users: ${error.message}` })
